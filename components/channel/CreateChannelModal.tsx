...
export function CreateChannelModal() {
  ...
  return (
    <Modal isOpen={open} onClose={close} title="Create Channel">
      <style>{`
        .modal-input {
          width: 100%;
          ...
          {(["TEXT", "VOICE"] as const).map((t) => (
            <button
              ...
              style={{
                ...
                padding: "0.75rem",
                borderRadius: "8px",
                border: `1px solid ${channelType === t ? "var(--accent)" : "var(--border)"}`,
                background: channelType === t ? "var(--accent-muted)" : "var(--surface-2)",
                color: channelType === t ? "var(--accent)" : "var(--text-secondary)",
                display: "flex",
                alignItems: "center",
                gap: "0.5rem",
                fontWeight: 500,
            ...
            >
              {t === "TEXT" ? <HashIcon size={18} /> : <VolumeIcon size={18} />}
              {t === "TEXT" ? "Text Channel" : "Voice Channel"}
            </button>
          ))}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ position: "relative" }}>
          <div style={{
            ...
            {channelType === "TEXT" ? <HashIcon size={16} /> : <VolumeIcon size={16} />}
          </div>
          <input
