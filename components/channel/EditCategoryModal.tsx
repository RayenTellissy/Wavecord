...
export function EditCategoryModal() {
  ...
  const open = isOpen && type === "editCategory";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await axios.patch(`/api/servers/${serverId}/categories/${categoryId}`, { name });
      close();
      router.refresh();
    } catch (err) {
  ...
  return (
    <Modal isOpen={open} onClose={close} title="Edit Category">
      <style>{`
        .modal-input {
          width: 100%;
