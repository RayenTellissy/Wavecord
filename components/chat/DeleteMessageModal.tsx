...
export function DeleteMessageModal() {
  ...
  async function handleDelete() {
    ...
    setError("");
    try {
      const path = isDM ? `/api/direct-messages/${messageId}` : `/api/messages/${messageId}`;
  ...
  return (
    <Modal isOpen={open} onClose={close} title="Delete Message" width={420}>
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        ...
        <p style={{ textAlign: "center", fontSize: "0.88rem", color: "var(--text-secondary)", lineHeight: 1.5 }}>
          This action <strong>cannot be undone</strong>. The message will be permanently removed.
        </p>
